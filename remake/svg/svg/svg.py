    ...
    def bbox(self):
        ...
        for el in self.items:
            try:
                ...
                print "Error while calculating bbox:", str(e)

        #print [x.__class__.__name__ for x in self.items]
    ...
    def append(self, element):
        for elt in element:
            ...
            if elt_class is None:
                if not elt.tag.startswith('{http:'):
...
#                    Arc(rx, ry, xrot, large_arc_flag, sweep_flag, Point(x, y)))
    ...
    #    xmin = min(s[0] for s in segments)
    #    xmax = max(s[0] for s in segments)
    #    ymin = min(s[1] for s in segments)
    #    ymax = max(s[1] for s in segments)
    #
    #    return Point(xmin, ymin), Point(xmax, ymax)
