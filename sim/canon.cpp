...
GcodeFile *_gcode_file;

void set_gcode_file(GcodeFile *file) {
    if (file) fprintf(stderr, "Canon GCODE file set to %s\n", file->get_filename().c_str());
    _gcode_file = file;
}
...
)
{
    ...
    start.point.x = _program_position_x;
    start.point.y = _program_position_z;
    start.point.z = _program_position_y;
...
#ifdef AA
    start.angles.x = _program_position_a;
    end.angles.x = a;
    _program_position_a SET_TO a;                 /*AA*/
#endif
#ifdef BB
    start.angles.y = _program_position_b;
    end.angles.y = b;
    _program_position_b SET_TO b;                 /*BB*/
#endif
#ifdef CC
    start.angles.z = _program_position_c;
    end.angles.z = c;
    _program_position_c SET_TO c;                 /*CC*/
#endif

    Path *p = new Path(start, end, -1);
    _gcode_file->current().set_path(p);
}
...
extern void SET_MOTION_CONTROL_MODE(CANON_MOTION_MODE mode)
{
    ...
    {
        //printf("SET_MOTION_CONTROL_MODE(CANON_EXACT_STOP)\n");
        _motion_mode SET_TO CANON_EXACT_STOP;
    }
    else if (mode IS CANON_EXACT_PATH)
    {
        //printf("SET_MOTION_CONTROL_MODE(CANON_EXACT_PATH)\n");
        _motion_mode SET_TO CANON_EXACT_PATH;
    }
    else if (mode IS CANON_CONTINUOUS)
    {
        //printf("SET_MOTION_CONTROL_MODE(CANON_CONTINUOUS)\n");
        _motion_mode SET_TO CANON_CONTINUOUS;
    }
    else {
        //printf("SET_MOTION_CONTROL_MODE(UNKNOWN)\n");
    }
}
...
void STOP_CUTTER_RADIUS_COMPENSATION() {
    //printf ("STOP_CUTTER_RADIUS_COMPENSATION()\n");
}

void START_SPEED_FEED_SYNCH() {
    //printf ("START_SPEED_FEED_SYNCH()\n");
}

void STOP_SPEED_FEED_SYNCH() {
    //printf ("STOP_SPEED_FEED_SYNCH()\n");
}
...
{
    pointf start, end, center;

    start.point.x = _program_position_x;
    start.point.y = _program_position_z;
    start.point.z = _program_position_y;
/*
    printf("ARC_FEED(fe=%.4f, se=%.4f, 1st_ax=%.4f, 2nd_ax=%.4f, rot=%d, aep=%.4f"
     #ifdef AA
         ", %.4f"                                  // AA
     #endif
     #ifdef BB
...
*/
    ...
    if (_active_plane IS CANON_PLANE_XY) {
        ...
        center.point.x = first_axis;
        center.point.y = axis_end_point;
        center.point.z = second_axis;
        ...
        center.point.x = axis_end_point;
        center.point.y = second_axis;
        center.point.z = first_axis;
        ...
        center.point.x = second_axis;
        center.point.y = first_axis;
        center.point.z = axis_end_point;
    }

    end.point.x = _program_position_x;
    end.point.y = _program_position_z;
    end.point.z = _program_position_y;
...
#endif
    ArcPath *p = new ArcPath(start, end, center, plane, rotation == -1 ? true : false, -1);
    _gcode_file->current().set_path(p);
}
...
)
{
    ...
    bool add = (_program_position_x != 0 || _program_position_y != 0 || _program_position_z != 0);
//    fprintf(stderr, "add = %d, (%f, %f, %f)\n", add ? 1 : 0, _program_position_x, _program_position_y, _program_position_z); 
...
void SPINDLE_RETRACT_TRAVERSE() {
    printf("SPINDLE_RETRACT_TRAVERSE()\n");
}

void START_SPINDLE_CLOCKWISE()
{
    printf("START_SPINDLE_CLOCKWISE()\n");
...
{
    printf("START_SPINDLE_COUNTERCLOCKWISE()\n");
    _spindle_turning SET_TO ((_spindle_speed IS 0) ? CANON_STOPPED :
...
{
    //printf("USE_TOOL_LENGTH_OFFSET(%.4f)\n", length);
}
...
//    _global_paths->set_bit(slot);
// FIXME gotta do something here
}


void SELECT_TOOL(int slot)
{
    printf("SELECT_TOOL(%d)\n", slot);
}
...
void DISABLE_FEED_OVERRIDE() {
    //printf("DISABLE_FEED_OVERRIDE()\n");
}

void DISABLE_SPEED_OVERRIDE() {
    //printf("DISABLE_SPEED_OVERRIDE()\n");
}

void ENABLE_FEED_OVERRIDE() {
    //printf("ENABLE_FEED_OVERRIDE()\n");
}

void ENABLE_SPEED_OVERRIDE() {
    //printf("ENABLE_SPEED_OVERRIDE()\n");
}
...
void PALLET_SHUTTLE()
{
    //printf("PALLET_SHUTTLE()\n");
}

void TURN_PROBE_OFF()
{
    printf("TURN_PROBE_OFF()\n");
}

void TURN_PROBE_ON()
{
    printf("TURN_PROBE_ON()\n");
}

void UNCLAMP_AXIS(CANON_AXIS axis)
{
    /*
      printf("UNCLAMP_AXIS(%s)\n",
        (axis IS CANON_AXIS_X) ? "CANON_AXIS_X" :
    (axis IS CANON_AXIS_Y) ? "CANON_AXIS_Y" :
    (axis IS CANON_AXIS_Z) ? "CANON_AXIS_Z" :
    (axis IS CANON_AXIS_A) ? "CANON_AXIS_A" :
